...
function renderCandidates() {
  ...
  ["candidate", "状态", "拓扑", "GPU", "TP/DP/PP", "DPA", "backend", "DSpark", "mem", "chunked", "best"].forEach(function(name) {
    html += "<th>" + escapeHtml(name) + "</th>";
  });
  html += "</tr></thead><tbody>";
  rows.forEach(function(c) {
    html += '<tr class="clickable" data-candidate="' + escapeHtml(c.id) + '">';
  ...
  box.querySelectorAll("tr[data-candidate]").forEach(function(row) {
    row.addEventListener("click", function() { openCandidate(row.dataset.candidate).catch(showError); });
  });
}
...
function renderTrials() {
  ...
  box.className = "table-wrap";
  var html = '<table><thead><tr>';
  ["task", "candidate", "状态", "class", "mode", "concurrency", "tok/s", "requests", "reasons"].forEach(function(name) {
    html += "<th>" + escapeHtml(name) + "</th>";
  });
  html += "</tr></thead><tbody>";
  rows.forEach(function(row, idx) {
    html += '<tr class="clickable" data-trial="' + idx + '">';
  ...
  box.querySelectorAll("tr[data-trial]").forEach(function(row) {
    row.addEventListener("click", function() { showTrialArtifacts(rows[Number(row.dataset.trial)]).catch(showError); });
  });
}
...
async function showTrialArtifacts(row) {
  var base = "";
  if (row.manifest && row.manifest.endsWith("/trial.json")) {
...
async function selectJob(jobId, fetchFresh) {
  ...
  if (!job) return;
  el("stop-job").disabled = job.status !== "running";
